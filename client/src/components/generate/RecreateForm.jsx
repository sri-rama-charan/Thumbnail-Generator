/**
 * Purpose: Form subcomponent for Recreate Mode in Studio.
 * 
 * Renders controls: Topic, Reference Image URL, Change Request inputs, 
 * Aspect Ratio, Style preset, Palette selection, and Visibility radio controls.
 */

import React from "react";
import { aspectRatios, palettes, styles } from "../../utils/constants";

const RecreateForm = ({ form, onChange, onPaletteClick, submitting }) => {
  return (
    <div className="space-y-5">
      {/* Title */}
      <div>
        <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400">
          Original Thumbnail Topic
        </label>
        <input
          type="text"
          name="title"
      ...
      <div>
        <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400">
          Source Image URL (pollinations.ai only)
        </label>
        <input
          type="url"
          name="sourceImage"
      ...
      <div>
        ...
        <textarea
          ...
          required
          rows="3"
          value={form.changeRequest}
          onChange={onChange}
          placeholder="Describe changes: e.g. Change the background to red neon, add big text reading 'AI CODE' next to the character."
      ...
      {/* Aspect Ratio & Style Preset Grid */}
